...
function [F, g] = fpgensinev(X_value, m)
% 返回线搜索的所需的X点处的函数值
% 调用
...
% Coder:    Su LiHui
    ...
    f1_tmp = (var_x(m) - sin(var_x(m-1)))^2 / c1;
    gf_f = diff(f1_tmp, var_x(m));
    g(m) = double(subs(gf_f, {var_x(m-1), var_x(m)}, {X_value(m-1), X_value(m)}));
end
