%% BFGS
function [y, reInfo] = BFGS(f, line_method, theta, X, lineFunc, varargin)
% 该函数使用 BFGS 算法求解x*.
%
% 调用
...
while abs(yk - yk_1)>theta && norm(gk_1, 2) > theta
    ...
    sk = xk - xk_1;
    delt_gk  = gk - gk_1;
    Hk = double(Hk_1 + (1+ (delt_gk*Hk_1* delt_gk')/(delt_gk*sk')) * ((sk'* sk)/(delt_gk*sk')) - ((sk'*delt_gk*Hk_1) + (Hk_1*delt_gk'*sk))/(delt_gk*sk'));
    [~,p] = chol(Hk);
    if p ~= 0
        disp('HK不正定。');
        Hk = eye(numOfvalue);
    ...
    if line_method.inextract ~= 2
        ...
        StepSize = minValue(f, Point, dk');
        info_search = [1, 1, 1];
     end
    % 下一个Xk的计算
    xk = Point + StepSize * dk';
    yk = double(subs(f, var_x, num2cell(xk)));
