function [y, reInfo] = PRPplus(f, line_method, theta, X, varargin)
% 该函数使用 FR 算法求解x*.
%
% 调用
...
if ~line_method.inextract
    [~, right] = L_JinTui(f, Point, d0, line_method.step);
else
...
while norm(gk_1, 2) > theta
    ...
    disp(double(gk));
    % PRP+ 方法的更新结果
    betak_1 = max( (gk*(gk' - gk_1')) / (gk_1*gk_1'), 0 ) ;
    disp('beta_k-1= ');
    disp(double(betak_1));
    ...
    Point = xk_1;
    if ~line_method.inextract
        [~, right] = L_JinTui(f, Point, dk, line_method.step);
    else
        right = line_method.inextract;
    end
    BeginPoint = Point;
